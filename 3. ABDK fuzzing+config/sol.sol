...
contract EchidnaTemplate {
    ...
    function div_test_not_commutative(int128 x, int128 y) public {
       // for some conditions assert is going to fail
       // when abs of x and y are same.
       // to resolve this we have few options
        // 1. Pre conditions
        //require(abs(x) != abs(y));
        // optimisation
        if(abs(x) == abs(y)){
...
// command to run: echidna sol.sol --contract EchidnaTemplate --config config.yaml 